...
//changed[j] == y ，你就可以选择字符串中的 子串 x 并以 z 的成本将其更改为 y 。 你可以执行 任意数量 的操作，但是任两次操作必须满足 以下两个
// 条件 之一 ： 
//
// 
// 在两次操作中选择的子串分别是 source[a..b] 和 source[c..d] ，满足 b < c 或 d < a 。换句话说，两次操作中选择的下标
// 不相交 。 
// 在两次操作中选择的子串分别是 source[a..b] 和 source[c..d] ，满足 a == c 且 b == d 。换句话说，两次操作中选择的
//下标 相同 。 
// 
//
...
//
//- 将子串 source[5..7] 从 "thh" 改为 "ghh" ，成本为 5 。可以执行此操作，因为下标 [5,7] 与第一次操作选中的下标不相交，
//且与第二次操作选中的下标相同。
//产生的总成本是 1 + 3 + 5 = 9 。
//可以证明这是可能的最小成本。
// 
...
// 
//
// Related Topics 图 字典树 数组 字符串 动态规划 最短路 👍 32 👎 0
